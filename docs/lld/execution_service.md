...
| `workflow.template.published` | `wf.template.events` | `workflow_id, workflow_key, version_id, version_number, artifact_hash, published_by` | Cache pre-warm: fetches the compiled plan and refreshes the `workflow_key → active version_id` map in the shared Valkey cache, recency-guarded against out-of-order redelivery. Fail-open — a warm-fetch failure logs and still returns `200`, since the plan loads lazily on the next instantiation | `template-sync-execution` | Acted on (benign failure) |
...
5. **Template published.** **Fetch** the compiled plan via `GetCompiledWorkflow`, **refresh** the plan cache and the `workflow_key → version_id` map guarded by the envelope-time recency check, **record** dedup and return `200` regardless of fetch outcome (fail-open).
...
1. **Template published.** Definition Service publishes a workflow version and emits `workflow.template.published`. Execution's consumer pre-warms its `workflow_key → active version_id` cache, recency-guarded, fail-open (§6.2). No instance exists yet.
...
```mermaid
flowchart TD
    TP[workflow.template.published<br/>cache pre-warm, §6.2] -.-> INST
...
| 19 | Template pre-warm cache updates are recency-guarded by envelope time, same skip semantics as tenant-lifecycle events | The `workflow_key → version_id` map is used by instantiation, not just display — an unguarded out-of-order redelivery could point new instances at a stale version. |
