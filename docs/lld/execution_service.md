...
- **`DefinitionService.GetCompiledWorkflow(tenant_id, workflow_version_id) → {workflow_id, version_id, version_number, status, is_valid, compiled_plan_json}`** — called at instantiation (`POST /instances`) and re-validated authoritatively inside `GetCompiledPlanActivity`; rejects a `DRAFT`/`ARCHIVED` version (`409 VERSION_NOT_PUBLISHED`) or an invalid one (`409 VERSION_INVALID`). The compiled plan is cached in Valkey by `(tenant_id, version_id)` (shared across API replicas), cache-aside with write-through on a miss (rev 1.37 — `workflow.template.published` and the eager-populator path it drove are gone entirely, not just superseded; see Appendix A.5 decision 19), fail-open on cache miss/unreachable-cache.
...
- **Type-scoped subpaths (rev 1.36; narrowed to 3, rev 1.37).** `POST /internal/events` stays registered as the catch-all — every type below, plus any future/unrecognized one, still dispatches through it. The shared consumer additionally posts 3 event categories directly to their own subpath, same auth/envelope/retry contract, purely a routing convenience: `/internal/events/delegation` (`delegation.started`/`.ended`), `/internal/events/user-profile` (`user.deleted`/`user.availability.changed`), `/internal/events/tenant` (`tenant.state.changed`). A fourth subpath, `/internal/events/workflow-template` (`workflow.template.published`), existed only from rev 1.36 to rev 1.37 — removed along with the event itself, not narrowed to zero traffic. A type the shared consumer doesn't recognize as belonging to one of these 3 categories still reaches the catch-all.
...
Plus `platform-pgcommon`'s own built-in collectors — `pgcommon_query_total`, `pgcommon_query_duration_seconds`, `pgcommon_pool_acquire_total`, `pgcommon_pool_acquire_duration_seconds`, `pgcommon_slow_query_total`, `pgcommon_retry_total`, `pgcommon_retry_exhausted_total` — activated via `observability.RegisterPGMetrics` (rev 1.38), plus the `PoolStatsCollector` pool gauges already registered for Definition Service, applied identically to both processes' pools. `db_tx_retry_total` (this package's own counter, above) is retired as of the same revision — it duplicated `pgcommon_retry_total`/`pgcommon_retry_exhausted_total` in intent and was never incremented anywhere in production code; those two now cover this signal instead.
...
| 6 | **Retired, rev 1.37** — ~~Template sync consumes only `workflow.template.published`, fail-open cache warm~~ | Three other candidate template events were never shipped in Definition Service's design; eligibility invalidation and archive guards are handled elsewhere. The one template event that *was* shipped, `workflow.template.published`, is itself now retired platform-wide (producer, consumer, schema, routes) — decision 19 covers the removal; there is no template-sync consumption of any kind left in this catalogue. |
...
| 19 | **Retired, rev 1.37 (rev 1.36 had marked this superseded, not yet fully retired).** Rev 1.36 removed `TemplateCachePrewarmer` and the `workflow_key → version_id` map it maintained (it turned out to have no reader anywhere in the codebase — a write-only cache entry, never actually consulted by instantiation as this decision assumed), but left the `workflow.template.published` event itself in place, doing nothing but payload validation and dedup-recording. Rev 1.37 removes the event in full — producer (Definition Service's `Publish()`/`Promote()`), consumer (this service's handler, route, eventschema), and the AsyncAPI/OpenAPI/schema entries on both sides — since a validate-and-dedup-only consumer wasn't judged useful enough to keep running. | The eager warm saved at most one `GetCompiledWorkflow` round-trip, on the very first instantiation after each publish, at the cost of standing prewarm/recency-gate machinery no other event type needed. `InstanceService.Start`'s own compiled-plan cache-aside read (keyed by `(tenant_id, version_id)`, unrelated to the removed map) was separately found to be read-only — never writing back on a miss, which would have left it permanently cold once the prewarmer was gone — and was fixed to write through, so it now stays warm on its own with no dedicated event needed to keep it that way. Once the prewarmer was gone, the event's only remaining job was payload validation and dedup-recording — not enough of a reason on its own to keep a whole platform-wide event (schema, routes, AsyncAPI/OpenAPI entries) alive. |
