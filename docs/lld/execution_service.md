...
`ExecutionService.PauseUserTasks(tenant_id, user_id)` is the safety-net RPC Definition Service calls on `department.membership.revoked`/`UserDeleted` — Execution runs no separate subscription for these.
...
`SUPERSEDED` exists precisely because a force-forward-bypassed task is neither a failure nor the assignee's own send-back — a distinct, dashboard-worth-distinguishing outcome. The interrupting-boundary case is the same distinction for a different trigger: the host task didn't fail and wasn't the assignee's own choice to defer — a boundary event just moved execution on without it. Unlike `workflow_instance.status`, there is no independent per-task failure mode: `FAILED` here means "swept up in the parent instance's terminal transition," never "this task failed on its own."
...
| `(event_type, (payload -> 'data' ->> 'task_id')) WHERE event_type IN ('workflow.task.sla-warning','workflow.task.sla-breached')` UNIQUE | `outbox_events` | DB-level backstop under `ExistsForTask`'s app-level SLA-event dedup check (§4.5) — not a query-serving index, a correctness constraint against two overlapping activity attempts both passing that check under READ COMMITTED |
...
| Inbound | `PauseUserTasks(tenant_id, user_id) → {}` | Internal IAM event consumer | The `department.membership.revoked`/`UserDeleted` safety-net pause vehicle |
...
Execution Service runs **no SQS consumer of its own**. The engine-wide shared Workflow-Events Consumer — the same thin service that already fronts Definition Service's inbound queues — subscribes Execution's inbound SNS topics and forwards each event envelope over HTTP to `POST /api/v1/internal/events`, exactly as it does for Definition Service.

- **Endpoint.** `POST /api/v1/internal/events`, under the `/api/v1/internal` route group, guarded by `RequireInternalToken` (`x-internal-token` vs. `INTERNAL_API_TOKEN`, required in production). Not exposed via the gateway — no gateway identity headers reach it. Tenant identity comes from the envelope's `tenant_id`, injected into the RLS GUC by the handler.
- **Type-scoped subpaths.** `POST /api/v1/internal/events` stays registered as the catch-all — every type below, plus any future/unrecognized one, still dispatches through it. The shared consumer additionally posts 3 event categories directly to their own subpath, same auth/envelope/retry contract, purely a routing convenience: `/api/v1/internal/events/delegation` (`DelegationStarted`/`.ended`), `/api/v1/internal/events/user-profile` (`UserDeleted`/`UserAvailabilityChanged`), `/api/v1/internal/events/tenant` (`TenantStateChanged`). A type the shared consumer doesn't recognize as belonging to one of these 3 categories still reaches the catch-all.
...
**Self-consumption, not IAM-sourced — and not yet confirmed reachable.** `handleWorkflowTaskCreated` (`internal_events_connector.go`) exists to receive Execution's own `workflow.task.created` back for connector/automation-typed tasks, pushing the connector dispatch onto the Stream `cmd/connector-worker` reads from. `api/asyncapi.yaml` documents this as a `self/task-dispatch` channel/`receiveSelfTaskDispatch` operation, but the handler only fires if Execution is actually subscribed to its own `workflow.task.created` topic through the platform event bus's subscription topology — the handler's own code comment flags this as unconfirmed, and the shared Workflow-Events Consumer's example routing table (`event_consumer/config/topology.example.yaml`) currently routes `workflow.task.created` only to `workflow-stream-gateway`, not back to Execution itself. Whether this self-consumption path is actually wired up in any real topology, or dispatches connector tasks some other way entirely, is unresolved — tracked as an open item (Appendix B), not asserted as working here.
...
| `department.membership.revoked` | `iam.membership.events` | — | **Not consumed.** Definition Service's own `POST /api/v1/internal/events` handler already calls Execution's `PauseUserTasks` gRPC synchronously as part of template invalidation; a second subscription here would double-pause the same user for no added safety | — | Not subscribed |
| `workflow.task.created` (self) | none confirmed — see §6.1's "self-consumption" note | `workflow_instance_id, task_id, node_key, department_id, connector_type?, resolved_inputs, output_mapping` | Connector/automation-typed tasks only (`connector_type` set): pushes the connector dispatch onto the Stream `cmd/connector-worker` reads. Non-connector tasks: recognized, no-op | — | Reachability unconfirmed (Appendix B) |
...
| `workflow.instance.paused` | `INSTANCE_PAUSED` | admin pause / `TenantStateChanged` loop / OOO loop | `started_by_user_id`, `initiator` (`admin`\|`tenant_state`\|`safety_net`\|`ooo`), `actor_user_id?` | Audit, Notif, Dashboard |
| `workflow.instance.resumed` | `INSTANCE_RESUMED` | admin resume / `TenantStateChanged`(→active) loop / OOO-return loop / DEGRADED park-loop exit | `started_by_user_id`, `initiator` (`admin`\|`tenant_state`\|`safety_net`\|`ooo`\|`degraded_recovery`), `actor_user_id?` | Audit, Notif, Dashboard |
...
`workflow.task.completed` **is** emitted for every task completion, matching the unconditional `outbox.Enqueue` rule and feeding the Dashboard Stream Gateway's optimistic-UI reconciliation. `workflow.instance.finished` counts Notification among its consumers, closing the loop for the platform's "workflow completed" notification. On the Tender queue specifically, `workflow.instance.finished` is a post-hoc finalization notification, never the signing trigger — signing happens synchronously before Complete.
...
3. **Instance record and workflow start.** On validation pass, the service layer writes the instance row (status `RUNNING`) synchronously, then starts the Temporal workflow keyed by `{tenantID}:{businessKey}`. `workflow.instance.started` is emitted (§6.4).
...
5. **Node → task creation.** For each user-task node reached, a task row is written (status `READY`) plus one assignment row per default assignee, and `workflow.task.created` is emitted with the task's identity, department, assignees, and any due/follow-up timestamps. SLA timers, if present, are armed.
6. **Assignee acts.** The assignment surfaces on the assignee's dashboard. The assignee completes the task; the assignment is marked done, and when every assignment on the task is done, the task transitions to `COMPLETED`, emitting `workflow.task.completed`.
...
| 11 | Wire event types are dotted-lowercase under `workflow.`; DB `event_type` stays SCREAMING_SNAKE; Pascal names remain doc-level aliases | Mirrors Definition Service's own shipped dotted-lowercase wire-event convention. |
...
| 20 | No `workflow.task.message-sent`/`TASK_MESSAGE_SENT` event exists in the catalogue | Not meaningful enough on its own to justify a dedicated event/schema entry — `send_task` dispatch stays fully traceable via `workflow_task`'s own row and the surrounding instance-timeline events (§6.4) |
...
| Deferred | Task actions (claim/complete/defer) never re-verify the assignee's live OOO/delegation/deleted status at the moment of the action — only identity (`x-user-id` matches the current assignee) is checked. Availability is enforced solely by the async `UserAvailabilityChanged`/`DelegationStarted`/`UserDeleted` handlers reacting after the fact (§6.2), leaving a window between the last-processed event and an in-flight claim/complete/defer call where a now-OOO, now-delegated-away, or now-deleted user could still act | Execution team — design a synchronous pre-check or explicitly accept the eventual-consistency window |
...
| Infra | **`workflow.task.created`'s self-consumption path (§6.1, §6.2) has no confirmed subscription topology.** `handleWorkflowTaskCreated` exists to receive Execution's own published event back and dispatch connector-typed tasks onto the Stream `cmd/connector-worker` reads — but nothing confirms Execution is actually subscribed to its own `workflow.task.created` topic; the shared Workflow-Events Consumer's example routing config currently routes this event only to `workflow-stream-gateway`. Until this is resolved one way or the other, whether connector task dispatch is reachable at all in a real deployment is unverified | Whoever owns the shared consumer's routing topology + Execution team |
...
-- SLA events (workflow.task.sla-warning/sla-breached) are audit-only (§3.1) —
